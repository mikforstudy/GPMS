<script>
    ...
    // 获取该教师名下所有结题报告
    async function fetchEndReports() {
        try {
            loading = true;
            const response = await axios.get(`http://127.0.0.1:8000/api/v1/enddocx/teacher/${username}`);
            endReports = response.data;
            console.log("获取的结题报告列表:", endReports);
        } catch (err) {
            error = `获取结题报告失败: ${err.message}`;
            console.error("获取结题报告失败:", err);
        } finally {
            loading = false;
        }
    }
    ...
    async function fetchReportDetail(studentId) {
        try {
            detailLoading = true;
            const response = await axios.get(`http://127.0.0.1:8000/api/v1/enddocx/teacher/detail/${studentId}`);
            selectedReport = response.data;
            teacherComment = selectedReport.content5 || ""; // 初始化教师评语
            reportStatus = selectedReport.status || "待审核"; // 初始化状态
            showDetailModal = true;
        } catch (err) {
            error = `获取结题报告详情失败: ${err.message}`;
            console.error("获取结题报告详情失败:", err);
        } finally {
            detailLoading = false;
        }
    }
    ...
    async function submitReview() {
        ...
        try {
            const response = await axios.put(`http://127.0.0.1:8000/api/v1/enddocx/teacher/${selectedReport.student_id}`, {
                status: reportStatus,
                content5: teacherComment
            });
            
            console.log("审核结果提交成功:", response.data);
            
            // 更新本地列表中的状态
            const index = endReports.findIndex(report => report.student_id === selectedReport.student_id);
            if (index !== -1) {
                endReports[index].status = reportStatus;
            }
...
<div class="container mx-auto p-4">
    <h1 class="text-2xl font-bold mb-6">审核学生结题报告</h1>
    ...
    {:else}
        <div class="overflow-x-auto">
            <table class="table w-full">
                ...
                <tbody>
                    {#each endReports as report, index}
                        <tr class="hover">
                            <td>{index + 1}</td>
        ...
        <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
            <div class="bg-white rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
                ...
                <div class="p-4 border-b sticky top-0 bg-white flex justify-between items-center">
                    <h2 class="text-xl font-semibold">结题报告详情</h2>
