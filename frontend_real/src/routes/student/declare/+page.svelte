        ...
        if (typeof window !== "undefined") {
            ...
            try {
                const response = await axios.get(
                    `http://127.0.0.1:8000/api/v1/selections/student/${username}`,
                );
    ...
    async function submitForm(event) {
        ...
        try {
            const response = await axios.post(
                "http://127.0.0.1:8000/api/v1/selections/student/",
                {
                    student_id,
                    teacher_id,
                    username,
...
<div class="max-w-lg mx-auto p-8 space-y-4 bg-white rounded-lg shadow-lg">
            ...
            <div class="mt-4">
                ...
                {#each existingSelections as selection}
                    <div class="card bg-base-100 shadow-sm mt-2 p-4">
                        ...
                        <p>
                            <strong>指导教师ID：</strong>{selection.teacher_id}
                        </p>
    ...
    {:else}
        <form onsubmit={submitForm} class="space-y-4">
            ...
            <div class="form-control">
                <label class="label" for="teacher_id">
                    <span class="label-text">教师姓名（teacher{teacher_id}）</span>
                </label>
            </div>
        ...
        {#if feedbackMessage}
            <div
                role="alert"
                class="alert {feedbackType === 'success'
                    ? 'alert-success'
                    : 'alert-error'} mt-4"
            >
