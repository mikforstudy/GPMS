...
# 教师：在projects表里根据teacher_name查询student_id，然后根据student_id查询每个学生最终文档文件，返回文件信息
@router.get("/teacher/{teacher_name}", summary="教师查询最终文档文件", description="教师查询最终文档文件")
...
# 教师下载学生的最终文档文件
@router.get("/teacher/download/{teacher_name}/{student_id}/{file_name}", summary="教师下载学生最终文档",
            description="教师下载指定学生的最终文档文件")
async def download_student_process_file(teacher_name: str, student_id: int, file_name: str):
    ...
    # 2. 构建文件路径并检查文件是否存在
    project_root = 'D:\\Code\\demo\\backend_real'
    file_path = os.path.join(project_root, 'output', str(student_id), 'end_file', file_name)
